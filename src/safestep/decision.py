...
class DecisionEngine:
    ...
    def ped_threshold(self, ped_wait_count: int, ped_avg_wait_s: float) -> float:
        threshold = (
    ...
    def pedestrian_pressure(self, ped_wait_count: int, ped_avg_wait_s: float) -> float:
        return ped_wait_count * 5 + ped_avg_wait_s

    def traffic_pressure(self, traffic_flow_rate: float, traffic_queue_length: int) -> float:
        return traffic_flow_rate + (traffic_queue_length * 8)
    ...
    def decide(self, signal: DecisionInput) -> DecisionOutcome:
        ...
        t_pressure = self.traffic_pressure(signal.traffic_flow_rate, signal.traffic_queue_length)
        if t_pressure >= self.config.traffic_suspend_threshold:
            return DecisionOutcome.SUSPEND_CROSSING

        p_pressure = self.pedestrian_pressure(signal.ped_wait_count, signal.ped_avg_wait_s)
        threshold = self.ped_threshold(signal.ped_wait_count, signal.ped_avg_wait_s)
