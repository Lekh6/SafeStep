...
- **Encryption**: AES-GCM plate encryption via `cryptography` (`security.py`, `evidence.py`)
- **Fail-safe**: runtime fallback to timer/normal controller mode if camera processing fails (`runtime.py`)

## Quick start

```bash
python -m pip install -U pytest cryptography
pytest -q
```

## Notes

- OpenCV and Ultralytics are optional runtime dependencies for live video/demo and are not required for tests.
